...
│   └── favicon.ico
├── src/                              # アプリケーションソースコード
│   ├── main.ts                       # エントリーポイント・認証フロー・画面ルーティング
│   ├── style.css                     # グローバルスタイル（Pico CSSインポート・CSS Variables定義）
│   ├── lib/                          # 外部サービスクライアント
│   │   └── supabaseClient.ts         # Supabaseクライアントのシングルトン
│   ├── ui/                           # UIレイヤー（画面描画・イベント処理）
│   │   ├── authScreen.ts             # ログイン画面
│   │   ├── logScreen.ts              # 記録画面
...
│   │   └── supabaseConditionLogRepository.ts           # Supabase実装
│   ├── utils/                        # ユーティリティ関数
...
├── .env.local.example                # 環境変数テンプレート（コミット対象）
├── package.json                      # 依存パッケージ定義
├── tsconfig.json                     # TypeScript設定
├── vite.config.ts                    # Vite設定（base: '/condition-logging/' を設定）
├── .gitignore
...
### `src/lib/`
外部サービスのクライアントインスタンスを管理する。

| ファイル | 役割 |
|---------|------|
| `supabaseClient.ts` | Supabaseクライアントのシングルトン。環境変数からURLとAnon Keyを読み込む |
...
| ファイル | 役割 |
|---------|------|
| `authScreen.ts` | ログインフォームの描画・サインイン処理・エラー表示 |
...
| ファイル | 役割 |
|---------|------|
| `conditionLogRepository.ts` | `ConditionLogRepository` インターフェースの定義（全メソッドがPromise返却） |
| `localStorageConditionLogRepository.ts` | LocalStorageを使ったMVP実装 |
| `supabaseConditionLogRepository.ts` | Supabaseを使った実装。RLS・UPSERTによる1日1件管理 |
